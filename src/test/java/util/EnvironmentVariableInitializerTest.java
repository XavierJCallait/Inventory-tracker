...
public class EnvironmentVariableInitializerTest {
    ...
    public void shouldStartWithURLPrefix() {
        Properties properties = EnvironmentVariableInitializer.getEnvironmentProperties();
        assertTrue(properties.getProperty("SERVER_URL").startsWith("jdbc:mysql://"), "SERVER_URL must start with jdbc:mysql://");
        assertTrue(properties.getProperty("DB_URL").startsWith("jdbc:mysql://"), "DB_URL must start with jdbc:mysql://");
    }

    @Test
    public void shouldHaveValidPasswordLength() {
        Properties properties = EnvironmentVariableInitializer.getEnvironmentProperties();
        assertTrue(properties.getProperty("DB_PASSWORD").length() >= 8, "Password must be at least 8 characters long");
    }

    @Test
    public void shouldHaveValidDatabaseName() {
        Properties properties = EnvironmentVariableInitializer.getEnvironmentProperties();
        assertTrue(properties.getProperty("DB_NAME").matches("^[a-zA-Z0-9_]+$"), "DB_NAME must contain only alphanumeric characters and underscores");
    }
}